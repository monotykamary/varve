...
Run `scripts/verify.sh` with Rust/Cargo, DuckDB v2, Node 22+, npm and Python 3 available. It includes both SDKs against a freshly built non-fault-enabled service, in addition to storage/transport fault regressions. Cargo packages are `varve-storage` and `varve-client`; the storage library and executable remain named `varve`. Keep fixtures small and deterministic; use `TempDir`, explicit event-time clocks, real process crash tests for publication boundaries, and ordinary Rust integration tests. Never hide a corruption/replay failure by dropping data or relaxing an assertion. Add a regression test before changing a durability contract.
