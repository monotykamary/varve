...
async def run():
    ...
    destination=Path('/results/sediment-recovery-'+mode+'.json')
    ...
    try:
        ...
        for run_id in ('sediment001','sediment002'):
        ...
        if mode=='after':
            before=json.loads(Path('/results/sediment-recovery-before.json').read_text())
