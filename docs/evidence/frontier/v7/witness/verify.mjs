...
const baseline=read('sediment001.json'),stress=read('sediment002.json');
...
assert.equal(stress.state,'overloaded');assert.equal(stress.manifest.total_committed_watermark_rows,1542000);assert.equal(stress.mixed_workload.offered_rows,600000);assert.equal(stress.mixed_workload.acknowledged_rows,542000);assert.equal(stress.mixed_workload.dropped_rows,58000);assert.equal(stress.mixed_workload.failed_or_ambiguous_rows,0);
...
const b=parse(before.metrics),m=parse(middle.metrics);assert.equal(m.varve_query_workers_spawned_total-b.varve_query_workers_spawned_total,23);assert.equal(m.varve_query_workers_reused_total-b.varve_query_workers_reused_total,561);
...
const last=parse(after.metrics);assert.equal(last.varve_query_workers_reused_total-m.varve_query_workers_reused_total,326);assert.equal(last.varve_query_workers_spawned_total-m.varve_query_workers_spawned_total,27);
...
let retained=0;for(const [id,count]of [['sediment001',550000],['sediment002',1542000]]){const f=ra.fingerprints[id];assert.equal(f.expected_watermark_rows,count);assert.equal(f.varve_raw.count,count);assert.equal(f.timescale_raw.count,count);assert.deepEqual(f.varve_raw,f.timescale_raw);assert.deepEqual(f.varve_aggregate,f.timescale_aggregate);retained+=count;}assert.equal(retained,2092000);
...
const local=read('local-qualification.json');assert.equal(hash(JSON.stringify(local.files)),local.source_digest);assert.equal(source.local_qualified_source,local.source_digest);assert.equal(local.rust_passed,321);assert.equal(local.ignored,1);for(const log of local.logs)assert.equal(hash(readFileSync(root+log.path)),log.sha256);for(const file of source.files){const qualified=local.files.find(f=>f.path===file.path);if(qualified)assert.equal(file.sha256,qualified.sha256);}
...
console.log(JSON.stringify({checked_files:manifest.files.length,source_files:source.files.length,latency_distributions:distributions,raw_samples:samples,oracle_verified_timed_queries:queries,baseline:'passed',large:'overloaded',pool_reuses:887,actual_restart_qualified:true,retained_common_rows_per_backend:2092000,active_benchmark_deployments:0,scope:'artifact checksums, source receipts and stored arithmetic; no local DB load or timing'}));
