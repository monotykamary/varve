...
for(const [helper,name]of cases){
 ...
 const disk=new Map([[root+'/before.json',JSON.stringify(before)],[root+'/activation-varve.json',JSON.stringify({deploymentId:'uploaded-varve'})]]);
 ...
 const writes=[],journals=[],mutations=[];let failure;
