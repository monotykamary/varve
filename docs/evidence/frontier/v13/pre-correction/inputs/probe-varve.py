import hashlib, http.client, json, os, pwd, re, subprocess, tempfile, time
...
def api(method,path,body=None,text=False):
    ...
    raw=response.read()
    result=raw.decode() if text else json.loads(raw)
...
assert result['source_manifest_sha256']=='f57157e947995785f0d226b2dd80791b100d32527418898e579d07962edb8a9d'
assert result['config_sha256']=='215f100b13ba1ba3d012d39117fa1895ed7978657434057a93390c6f359aa089'
assert result['region']=='asia-southeast1-eqsg3a'
assert result['data_dir']=='/data/probes/accounting-s13-r1'
assert result['cpu_max']=='200000 100000' and int(result['memory_max'])==1999998976
result['source_qualification']=json.loads(manifest)['local_qualified_source']
assert result['source_qualification']=='58f405adf7d472fdccf0011bc0bcc3f041201f9792ec1a7ab372002b1803944a'
result['tables']=api('GET','/v1/tables')
assert result['tables']==[]
result['metrics']=api('GET','/metrics',text=True)
...
result['phase_names']=re.findall(r'^varve_phase_duration_seconds_count\{phase="([^"]+)"\} [0-9]+$',result['metrics'],re.M)
assert sorted(result['phase_names'])==sorted(phases), 'Wrong fixed phase registry'
...
assert config['checkpoint_frozen_prefix'] is True
