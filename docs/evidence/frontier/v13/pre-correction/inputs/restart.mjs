...
const expected={varve:JSON.parse(readFileSync(root+'/activation-varve.json','utf8')).deploymentId,timescale:JSON.parse(readFileSync(root+'/redeployed.json','utf8')).deployments.timescale};
const proof=JSON.parse(readFileSync(root+'/recovery-before.json','utf8'));
for(const run of ['acct101','acct102']){const report=JSON.parse(readFileSync(root+'/'+run+'.json','utf8'));if(!['passed','overloaded'].includes(report.state)||report.mixed_workload.failed_or_ambiguous_rows!==0||!proof.fingerprints[run])throw Error('Missing qualified common-frontier proof');}
