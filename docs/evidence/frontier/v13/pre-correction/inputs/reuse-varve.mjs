...
execFileSync('node',[root+'/freeze-helpers.mjs','--check'],{stdio:'pipe'});
execFileSync('node',[root+'/verify-ready.mjs'],{stdio:'pipe'});
if(existsSync(root+'/activation-varve-intent.json')||existsSync(root+'/activation-varve.json'))throw Error('Varve activation already attempted; reconcile without retry');
const configured=JSON.parse(readFileSync(root+'/configured.json','utf8'));
if(configured.variables.varve.VARVE_DATA_DIR!=='/data/probes/accounting-s13-r1'||configured.variables.varve.VARVE_BENCH_CONFIG!==readFileSync(root+'/benchmark-config.json','utf8').trim()||configured.variables.timescale.PGDATA!=='/var/lib/postgresql/data/accounting-s13-r1')throw Error('Exact retry configuration required');
...
for(const role of Object.keys(roles))if(current[role].instance.activeDeployments.length)throw Error('All owned compute must be cold');
const old='a88e2c35-596f-41d4-b383-06857bf43108';
const previous=api('query{varve:deployment(id:'+JSON.stringify(old)+'){id status serviceId environmentId projectId}}').varve;
if(!previous||previous.id!==old||previous.status!=='REMOVED'||previous.serviceId!==roles.varve||previous.environmentId!==environmentId||previous.projectId!==projectId)throw Error('Pinned Varve image ownership/state mismatch');
writeFileSync(root+'/activation-varve-intent.json',JSON.stringify({at:new Date().toISOString(),previous,current,usePreviousImageTag:true},null,2)+'\n',{mode:0o600,flag:'wx'});
const response=api('mutation($id:String!){deploymentRedeploy(id:$id,usePreviousImageTag:true){id status serviceId environmentId projectId}}',{id:old});
writeFileSync(root+'/activation-varve-response.json',JSON.stringify({at:new Date().toISOString(),response},null,2)+'\n',{mode:0o600,flag:'wx'});
const next=response.deploymentRedeploy;
if(!next||typeof next.id!=='string'||!next.id||next.id===old||next.serviceId!==roles.varve||next.environmentId!==environmentId||next.projectId!==projectId||!['QUEUED','INITIALIZING','WAITING','BUILDING','DEPLOYING','SUCCESS'].includes(next.status))throw Error('New Varve deployment ownership/state mismatch; reconcile retained response');
writeFileSync(root+'/activation-varve.json',JSON.stringify({at:new Date().toISOString(),deploymentId:next.id,submitted:next,previous,usePreviousImageTag:true},null,2)+'\n',{mode:0o600,flag:'wx'});
console.log(JSON.stringify({submitted:next,previous_image:old,usePreviousImageTag:true,not_yet_ready:true}));
