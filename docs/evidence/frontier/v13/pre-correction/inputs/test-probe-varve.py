import copy, hashlib, http.client, io, json, os, pwd, subprocess, time, tempfile
...
for case in cases:
    ...
    files={'/usr/share/doc/varve/benchmark-source.json':b'{}' if case=='manifest' else manifest,'/data/probes/benchmark.json':b'{}' if case=='config' else config,'/sys/fs/cgroup/cpu.max':b'max 100000' if case=='cpu' else b'200000 100000','/sys/fs/cgroup/memory.max':b'1' if case=='memory' else b'1999998976','/data/probes/accounting-s13-r1/manifest.bin':b'VARVEM02'+json.dumps(runtime['initial_root']).encode()+b'0'*32}
    requests=[]
    class Connection:
    ...
    env={'PATH':'/fixture','VARVE_API_TOKEN':'fixture-only-token','VARVE_DATA_DIR':'/wrong' if case=='directory' else '/data/probes/accounting-s13-r1','RAILWAY_REPLICA_REGION':'wrong' if case=='region' else 'asia-southeast1-eqsg3a'}
