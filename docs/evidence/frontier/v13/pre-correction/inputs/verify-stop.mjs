...
const recorded={varve:JSON.parse(readFileSync(root+'/activation-varve.json','utf8')).deploymentId};
if(existsSync(root+'/redeployed.json')){const refs=JSON.parse(readFileSync(root+'/redeployed.json','utf8')).deployments;for(const [role,id]of Object.entries(refs)){if(!['timescale','driver'].includes(role))throw Error('Invalid reference redeploy receipt');recorded[role]=id;}}
if(Object.keys(expected).length!==Object.keys(recorded).length||Object.entries(recorded).some(([role,id])=>!id||expected[role]!==id))throw Error('Cleanup receipt differs from exact owned deployments');
...
for(const role of Object.keys(roles))if(current[role].instance.activeDeployments.length)throw Error('Compute not fully removed: '+role);
for(const [role,id]of Object.entries(expected)){const info=deployments[role];if(!info||info.id!==id||info.status!=='REMOVED'||info.serviceId!==roles[role]||info.environmentId!==environmentId||info.projectId!==projectId||current[role].instance.activeDeployments.length)throw Error('Compute not fully removed: '+role);}
