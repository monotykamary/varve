...
const newVarve=JSON.parse(readFileSync(root+'/activation-varve.json','utf8')).deploymentId;
...
for(const [role,id]of Object.entries(old)){const deployment=oldState[role];if(!deployment||deployment.id!==id||deployment.status!=='REMOVED'||deployment.serviceId!==roles[role]||deployment.environmentId!==environmentId||deployment.projectId!==projectId||current[role].instance.activeDeployments.length)throw Error('Old deployment ownership/state mismatch');}
