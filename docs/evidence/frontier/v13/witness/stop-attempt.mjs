...
const expected={varve:JSON.parse(readFileSync(root+'/activation-varve.json','utf8')).deploymentId};
if(existsSync(root+'/redeployed.json')){const recorded=JSON.parse(readFileSync(root+'/redeployed.json','utf8')).deployments;for(const [role,id]of Object.entries(recorded)){if(!['timescale','driver'].includes(role)||typeof id!=='string'||!id)throw Error('Invalid reference redeploy receipt');expected[role]=id;}}
if(buildOnly&&Object.keys(expected).length!==1)throw Error('Build-only cleanup cannot discard driver results');
if(!buildOnly&&!aborting)for(const id of ['acct101','acct102']){const report=JSON.parse(readFileSync(root+'/'+id+'.json','utf8'));if(!report.finished_at)throw Error('Workload unfinished');}
