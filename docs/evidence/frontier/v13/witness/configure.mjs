...
if(existsSync(root+'/configuration-intent.json')||existsSync(root+'/configured.json'))throw Error('Configuration already recorded; inspect existing proof');
...
const variables={varve:{VARVE_DATA_DIR:'/data/probes/accounting-s13-r1',VARVE_BENCH_CONFIG:readFileSync(root+'/benchmark-config.json','utf8').trim()},timescale:{PGDATA:'/var/lib/postgresql/data/accounting-s13-r1'}};
...
console.log(JSON.stringify({configured:completed,readback_verified:true,original_unchanged:true,compute_started:false,explicit_format:'v2 opt-in',derived_budget_bytes:536870912,fresh_directory_suffix:'accounting-s13-r1'}));
