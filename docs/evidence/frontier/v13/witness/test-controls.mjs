...
const old={timescale:'83232bed-bae0-4503-8195-23a6a527ca9e',driver:'f0bcb2d9-3faf-46c0-97bf-5b27807dc6ef'};
...
for(const test of cases){
 ...
 else{assert.equal(failure,undefined);assert.deepEqual(mutations,['timescale','driver']);assert.deepEqual(writes.at(-1).value.deployments,{timescale:'new-timescale',driver:'new-driver'});assert(!writes.some(write=>write.path.endsWith('/activation-varve.json')));}
}
for(const mode of ['success','parse_failure','failed_workload','wrong_launch','varve_probe_failure','varve_check_failure','root_launch','wrong_launch_gid']){
 ...
 const execute=async(command,args)=>{
  ...
  if(name==='status.mjs')return {stdout:JSON.stringify({deployments:Object.fromEntries(['varve','timescale','driver'].map(role=>[role,{status:'SUCCESS'}])),readiness:{varve:'READY',timescale:'READY',driver:'READY'}})};
  if(name==='ssh.mjs'){
   ...
   if(script==='launch.py')return {stdout:JSON.stringify({at:new FixtureDate().toISOString(),run_id:id,mode:args[4],uid:mode==='root_launch'?0:10001,gid:mode==='wrong_launch_gid'?0:10001,effective_driver_files:mode==='wrong_launch'?{}:prepared.effective_driver_files,driver_path:'/results/driver-accounting-s13-r1/benchmark.py'})};
  ...
  if(name==='unpack.mjs'){
   const id=args[1],phase=id==='acct101'?'baseline':'stress';
 ...
 if(earlyFailure){assert(!calls.some(call=>call[0]==='redeploy.mjs'));assert(!calls.some(call=>call[0]==='ssh.mjs'&&call[1]!=='varve'));assert(!disk.has(root+'/acct101.launch.json'));}
 else{assert.equal(calls[2][0],'check-varve.mjs');assert.equal(calls[3][0],'redeploy.mjs');assert(disk.has(root+'/acct101.launch.json'));assert(calls.findIndex(call=>call[2]===root+'/install-driver.py')<calls.findIndex(call=>call[0]==='check-driver.mjs'));}
 ...
 if(!expectedFailure){assert(disk.has(root+'/acct102.launch.json'));assert(restarted);} else {assert(!disk.has(root+'/acct102.launch.json'));assert(!restarted);}
}
const helpers=['prepare-reuse.mjs','reuse-varve.mjs','configure.mjs','remote.mjs','redeploy.mjs','status.mjs','ssh.mjs','launch.py','unpack.mjs','stop-attempt.mjs','probe-varve.py','check-driver.mjs','probe-driver.py','collect.py','recovery.py','check-recovery.mjs','metrics.py','resources.sh','restart.mjs','verify-stop.mjs','run-campaign.mjs','install-driver.py','check-varve.mjs','verify-stage.mjs','analyze-phases.mjs','install-driver.py.template'];
