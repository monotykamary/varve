...
async def run():
    ...
    destination=Path('/results/resident-recovery-'+mode+'.json')
    ...
    try:
        ...
        for run_id in ('resident001','resident002'):
        ...
        if mode=='after':
            before=json.loads(Path('/results/resident-recovery-before.json').read_text())
