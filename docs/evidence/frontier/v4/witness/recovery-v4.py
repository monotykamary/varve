...
async def run():
    ...
    destination=Path('/results/frontier-v4-recovery-'+mode+'.json')
    ...
    try:
        ...
        for run_id in ('frontier003','frontier004'):
        ...
        if mode=='after':
            before=json.loads(Path('/results/frontier-v4-recovery-before.json').read_text())
