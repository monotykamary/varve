...
async def run():
    ...
    destination=Path('/results/prefix-recovery-'+mode+'.json')
    ...
    try:
        ...
        for run_id in ('prefix001','prefix002'):
            source_path=Path('/results/prefix001.recovery-reference.json') if run_id=='prefix001' else Path('/results/'+run_id+'.json')
        ...
        if mode=='after':
            before=json.loads(Path('/results/prefix-recovery-before.json').read_text())
