...
const ids={varve:JSON.parse(readFileSync(root+(existsSync(root+'/upload-varve-final.jsonl')?'/upload-varve-final.jsonl':'/upload-varve.jsonl'),'utf8')).deploymentId};
if(existsSync(root+'/redeployed.json'))Object.assign(ids,JSON.parse(readFileSync(root+'/redeployed.json','utf8')).deployments);
const deployments=api('query{'+Object.entries(ids).map(([role,id])=>role+':deployment(id:'+JSON.stringify(id)+'){id status createdAt}').join(' ')+'}');
