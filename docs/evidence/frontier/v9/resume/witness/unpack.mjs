...
const root='/tmp/varve-prefix2.2Q0Icf/resume-stress',id=process.argv[2];
if(!/^prefix00[12]$/.test(id))throw Error('Unknown run ID');
...
summary.initial=Object.fromEntries(Object.entries(report.initial_ingest??{}).map(([backend,data])=>[backend,{rows:data.rows,rows_per_second:data.rows_per_second,seconds:data.seconds,ack_p95_ms:data.ack_latency_ms.summary.p95_ms}]));
if(report.mixed_workload){const m=report.mixed_workload;summary.mixed={offered:m.offered_rows,acknowledged:m.acknowledged_rows,dropped:m.dropped_rows,failed_or_ambiguous:m.failed_or_ambiguous_rows,seconds:m.seconds,varve_ack_p95_ms:m.varve_ack_latency_ms.summary.p95_ms,timescale_ack_p95_ms:m.timescale_ack_latency_ms.summary.p95_ms,varve_read_p95_ms:m.varve_concurrent_read_ms.summary.p95_ms,timescale_read_p95_ms:m.timescale_concurrent_read_ms.summary.p95_ms,read_samples:m.varve_concurrent_read_ms.summary.samples};}
summary.query_p95_ms=Object.fromEntries(Object.entries(report.query_stages??{}).map(([stage,data])=>[stage,Object.fromEntries(Object.entries(data.backends).map(([backend,queries])=>[backend,Object.fromEntries(Object.entries(queries).map(([name,q])=>[name,q.latency_ms.summary.p95_ms]))]))]));
