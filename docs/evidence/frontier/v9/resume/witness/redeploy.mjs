...
for(const [role,deployment]of Object.entries(oldState))if(deployment.status!=='REMOVED'||deployment.serviceId!==roles[role]||deployment.environmentId!==environmentId||deployment.projectId!==projectId||current[role].instance.activeDeployments.length)throw Error('Old deployment ownership/state mismatch');
writeFileSync(root+'/redeploy-intent.json',JSON.stringify({at:new Date().toISOString(),oldState,usePreviousImageTag:true},null,2)+'\n',{mode:0o600});
const deployments={};
for(const [role,id]of Object.entries(old)){
 ...
 if(role==='varve')writeFileSync(root+'/upload-varve.jsonl',JSON.stringify({deploymentId:next.id,reused_image_from:id})+'\n',{mode:0o600});
