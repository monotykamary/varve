...
for(const run of ['prefix001','prefix002']){const report=JSON.parse(readFileSync(root+'/'+run+'.json','utf8'));if(!['passed','overloaded'].includes(report.state)||report.mixed_workload.failed_or_ambiguous_rows!==0||!proof.fingerprints[run])throw Error('Missing qualified common-frontier proof');}
