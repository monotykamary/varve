...
root=Path('/tmp/varve-prefix2.2Q0Icf/resume-stress')
...
def parse(text):
    ...
    for line in text.splitlines():
        ...
        if line.startswith(('varve_query_workers_', 'varve_query_resident_', 'varve_ingest_')):
            key,value=line.rsplit(' ',1)
            workers[key]=float(value)
    return phases,workers
bp,bw=parse(before['metrics']); ap,aw=parse(after['metrics'])
assert len(ap)==22 and ap.keys()==bp.keys()
rows=[]
for name in ap:
...
print(json.dumps({'scope':'resumed stress interval includes untimed boundary delay; phase intervals overlap, not exclusive CPU/wall time; working-byte gauge is not a peak','before_at':before['at'],'after_at':after['at'],'phases':sorted(rows,key=lambda row:row['cumulative_seconds'],reverse=True),'workers':workers,'sizes':{key:after['status'][key] for key in ('metadata_bytes','control_root_bytes','derived_encoded_bytes','derived_resident_bytes','derived_working_bytes')},'memory_peak':after['memory_peak'],'memory_events':after['memory_events']}))
