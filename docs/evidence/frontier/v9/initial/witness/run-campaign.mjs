...
const execute=promisify(execFile),root='/tmp/varve-prefix2.2Q0Icf';
...
if(resuming){
  ...
  assert.equal(read('prefix001.json').state,'running');
...
try{
  ...
  await workload('prefix001','baseline',1200,'baseline',resuming);
  await workload('prefix002','stress',600,'stress');
  await Promise.all([
