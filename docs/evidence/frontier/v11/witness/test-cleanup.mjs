...
for(const test of cases){
 ...
 const sandbox={process:{argv:test.aborting?['node','stop','--abort']:['node','stop'],env:{}},console:{log(){}},
  ...
  readFileSync(path){if(path.endsWith('/before.json'))return JSON.stringify(before);if(path.endsWith('/upload-varve.jsonl'))return JSON.stringify({deploymentId:ids.varve});if(path.endsWith('/redeployed.json'))return JSON.stringify({deployments:recorded});if(/\/diag10[12]\.json$/.test(path))return JSON.stringify({finished_at:test.unfinished?null:'finished'});throw Error('Unexpected fixture read '+path);},
