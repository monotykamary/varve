...
const execute=promisify(execFile),root='/tmp/varve-diagnostic.KOUrtH/retry-config';
...
async function workload(id,mode,ceiling,phase){
  ...
  assert.equal(launch.driver_path,'/results/driver-diagnostic-kourth-2/benchmark.py');
...
try{
  ...
  await workload('diag101','baseline',1200,'baseline');
  await workload('diag102','stress',600,'stress');
  await Promise.all([
