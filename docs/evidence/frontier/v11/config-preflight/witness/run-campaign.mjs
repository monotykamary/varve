...
const execute=promisify(execFile),root='/tmp/varve-diagnostic.KOUrtH';
...
async function workload(id,mode,ceiling,phase){
  ...
  assert.equal(launch.driver_path,'/results/driver-diagnostic-kourth/benchmark.py');
...
try{
  ...
  await workload('diag001','baseline',1200,'baseline');
  await workload('diag002','stress',600,'stress');
  await Promise.all([
