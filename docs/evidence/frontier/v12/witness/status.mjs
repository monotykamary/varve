...
const ids={varve:JSON.parse(readFileSync(root+'/upload-varve.jsonl','utf8')).deploymentId};
...
for(const [role,id]of Object.entries(ids)){
 ...
 if(active.length>1||active.some(d=>d.id!==id)||(info.status==='SUCCESS'&&(active.length!==1||active[0].status!=='SUCCESS')))throw Error('Unexpected active deployment '+role);
}
...
writeFileSync(root+'/status-latest.json',JSON.stringify({at:new Date().toISOString(),deployments,state:current},null,2)+'\n',{mode:0o600});
console.log(JSON.stringify({deployments,original_unchanged:true,active:Object.fromEntries(Object.keys(roles).map(role=>[role,current[role].instance.activeDeployments]))}));
