...
const execute=promisify(execFile),root='/tmp/varve-diagnostic.KOUrtH/retry-config/s13-pilot';
...
async function workload(id,mode,ceiling,phase){
  ...
  assert.equal(launch.driver_path,'/results/driver-accounting-s13/benchmark.py');
...
try{
  ...
  await ready(['timescale','driver'],300);
  await Promise.all([
  ...
  await workload('acct001','baseline',1200,'baseline');
  await workload('acct002','stress',600,'stress');
  await Promise.all([
