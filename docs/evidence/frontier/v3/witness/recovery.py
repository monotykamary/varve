...
async def run():
    ...
    destination=Path('/results/frontier-recovery-'+mode+'.json')
    ...
    try:
        ...
        for run_id in ('frontier001','frontier002'):
        ...
        if mode=='after':
            before=json.loads(Path('/results/frontier-recovery-before.json').read_text())
