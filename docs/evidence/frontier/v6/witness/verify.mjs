...
const baseline=read('reuse003.json'),stress=read('reuse004.json');
...
assert.equal(stress.state,'overloaded');assert.equal(stress.manifest.total_committed_watermark_rows,1522000);assert.equal(stress.mixed_workload.offered_rows,600000);assert.equal(stress.mixed_workload.acknowledged_rows,522000);assert.equal(stress.mixed_workload.dropped_rows,78000);assert.equal(stress.mixed_workload.failed_or_ambiguous_rows,0);
...
const b=parse(before.metrics),m=parse(middle.metrics);assert.equal(m.varve_query_workers_spawned_total-b.varve_query_workers_spawned_total,13);assert.equal(m.varve_query_workers_reused_total-b.varve_query_workers_reused_total,570);
...
const last=parse(after.metrics);assert.equal(last.varve_query_workers_reused_total-m.varve_query_workers_reused_total,322);assert.equal(last.varve_query_workers_spawned_total-m.varve_query_workers_spawned_total,29);
...
let retained=0;for(const [id,count]of [['reuse003',550000],['reuse004',1522000]]){const f=ra.fingerprints[id];assert.equal(f.expected_watermark_rows,count);assert.equal(f.varve_raw.count,count);assert.equal(f.timescale_raw.count,count);assert.deepEqual(f.varve_raw,f.timescale_raw);assert.deepEqual(f.varve_aggregate,f.timescale_aggregate);retained+=count;}assert.equal(retained,2072000);
...
const local=read('local-qualification.json');assert.equal(hash(JSON.stringify(local.files)),local.source_digest);assert.equal(source.local_qualified_source,local.source_digest);assert.equal(local.rust_passed,297);assert.equal(local.ignored,1);for(const log of local.logs)assert.equal(hash(readFileSync(root+log.path)),log.sha256);for(const file of source.files){const qualified=local.files.find(f=>f.path===file.path);if(qualified)assert.equal(file.sha256,qualified.sha256);}
const log=readFileSync(root+'local-qualification.log','utf8');assert(log.includes('ALL_REQUESTED_GATES_PASSED'));let tests=0;for(const line of log.split('\n'))if(line.startsWith('test result: ok.')&&line.includes('; 0 filtered out;'))tests+=Number(line.split(' ').filter(Boolean)[3]);assert.equal(tests,297);
...
console.log(JSON.stringify({checked_files:manifest.files.length,source_files:source.files.length,latency_distributions:distributions,raw_samples:samples,oracle_verified_timed_queries:queries,baseline:'passed',large:'overloaded',pool_reuses:892,actual_restart_qualified:true,retained_common_rows_per_backend:2072000,active_benchmark_deployments:0,scope:'artifact checksums, source receipts and stored arithmetic; no local DB load or timing'}));
