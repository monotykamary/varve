...
if(existsSync(root+'/restart-request.json'))throw Error('Restart already requested; inspect exact deployment/process evidence');
const expected={varve:JSON.parse(readFileSync(root+'/upload-varve.jsonl','utf8')).deploymentId,timescale:JSON.parse(readFileSync(root+'/redeployed.json','utf8')).deployments.timescale};
const proof=JSON.parse(readFileSync(root+'/recovery-before.json','utf8'));
for(const run of ['scoped001','scoped002']){const report=JSON.parse(readFileSync(root+'/'+run+'.json','utf8'));if(!['passed','overloaded'].includes(report.state)||report.mixed_workload.failed_or_ambiguous_rows!==0||!proof.fingerprints[run])throw Error('Missing qualified common-frontier proof');}
...
for(const [role,id]of Object.entries(expected)){const active=current[role].instance.activeDeployments;if(active.length!==1||active[0].id!==id||active[0].status!=='SUCCESS')throw Error('Unexpected active deployment');}
writeFileSync(root+'/restart-request.json',JSON.stringify({at:new Date().toISOString(),expected,reason:'Actual process restart on the same retained volumes and exact deployed images.'},null,2)+'\n',{mode:0o600});
const result=api('mutation{'+Object.entries(expected).map(([role,id])=>role+':deploymentRestart(id:'+JSON.stringify(id)+')').join(' ')+'}');
writeFileSync(root+'/restart-response.json',JSON.stringify({at:new Date().toISOString(),result})+'\n',{mode:0o600});
console.log(JSON.stringify({restart_requested:result,physical_recovery_verification_pending:true}));
