...
impl Database {
    ...
    fn prepare_group_attempts(
                ...
                if !overlay.items.is_empty() {
                    ...
                    let publication: Result<()> = match publication {
                        Ok((encoded, generation, published_now)) => {
                            ...
                            drop(s);
                            return Ok((results, Some(PreparedPublication {
                                pending, encoded, sequence, generation, published_now,
                                floors_before, commit,
                            })));
                        }
                        Err(error) => {
                            drop(overlay);
        ...
        PreparedEpoch { db: self.clone(), requests, results, publication, durable }
    }
    ...
    pub fn scan(
        ...
        let _output_memory = self.inner.raw_memory.reserve(raw_memory::row_charge(
        ...
        let (segments, cutoff_us, mut output, pin) = {
            ...
            let selected_bytes = selected.clone().try_fold(0usize, |bytes, row| {
                bytes
                    .checked_add(row.row.estimated_bytes())
                    .context("scan byte accounting overflow")
            })?;
            ensure!(
                selected_bytes <= self.inner.config.query_max_output_bytes,
                "scan output budget exceeded"
            );
            let output: Vec<StoredRow> = selected.cloned().collect();
            let shard = tenant
                .zip(series)
        ...
        let (_raw_working, snapshots, mut resident_snapshot, pin, catalog, _derived_working) = {
            ...
            for (name, table) in &s.catalog.tables {
                ...
                let descriptors = table
                    ...
                    .cloned()
                    .collect::<Vec<_>>();
                // Reserve before legacy flattening or host-side SQL staging.
                // Native DuckDB vectors themselves belong to DuckDB's allocator.
                let raw_bytes = s
                    .hot
                    .get(name)
                    .into_iter()
                ...
                if raw_selected && (raw_bytes > 0 || self.inner.native_runtime.is_some()) {
                    raw_working.push(self.inner.raw_memory.reserve(
                        if self.inner.native_runtime.is_some() {
                            raw_bytes.saturating_mul(8).saturating_add(
                                self.inner.config.query_threads.saturating_mul(64 * 1024),
                            )
                        } else {
...
fn resolve_terminal_durable(
    ...
    for (request, outcome) in requests.iter().zip(durable) {
        ...
        let Some(table) = s.catalog.tables.get(&item.table) else { continue };
        if !table.receipts.contains_key(&item.request_id) { continue; }
        ...
        if outcome.as_ref().is_some_and(Result::is_ok) {
            ...
            *outcome = Some(group_retry_with_floors(&s.catalog, &mut floors, item, &item.digest)
...
fn preflight_append(
    ...
    let reservation = s.raw_memory.reserve(raw_memory::row_charge(bytes))?;
    ...
    let updates = aggregate_updates(table, &stored, update_budget, |key| {
    ...
    Ok(PreparedAppend {
        ...
        receipt,
        updates,
        batch: resident_batch(stored, bytes),
        bytes,
        metadata_bytes,
    })
}
...
pub(crate) fn read_raw_segment(
    ...
    let reserve = |bytes| {
        if working {
            ...
            inner.raw_memory.reserve(bytes)
        }
    };
