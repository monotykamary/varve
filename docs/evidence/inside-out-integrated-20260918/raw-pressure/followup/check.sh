...
name=$1
shift
manifest=${SOURCE_MANIFEST:-$e/source-files.sha256}
sha256sum -c "$manifest" > "$e/$name.source-before.log"
sha256sum "$manifest" > "$e/$name.manifest.sha256"
set +e
"$@" > "$e/$name.log" 2>&1
rc=$?
set -e
sha256sum -c "$manifest" > "$e/$name.source-after.log"
