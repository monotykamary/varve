...
output = Path('/results/ior6_atomic_01.json')
with output.open('x'):
    pass
credentials = load_credentials()
report = {'state': 'running', 'run_id': 'ior6_atomic_01', 'driver_artifact': artifact_metadata(), 'probe_sha256': hashlib.sha256(Path(__file__).read_bytes()).hexdigest()}
...
async def run():
    ...
    try:
        ...
        rejected = replace(measurement(0, base), value=42.0)
        try:
