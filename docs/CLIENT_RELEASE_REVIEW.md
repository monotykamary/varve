...
A deterministic fixture regression reserves a PG address without listening and proves HTTP-only readiness is insufficient. It failed before the helper correction. The helper now waits for every configured listener; all 18 transport tests and strict targeted Clippy pass. This adds one test to the inventory and does not change production code or weaken frame, authentication or durability assertions. Full Linux CI [35055893569](https://github.com/monotykamary/varve/actions/runs/35055893569) passed on `45d7c014ea9070e6a2f299a752c121c748434762`: 182 storage/server tests, 24 Rust SDK tests, 18 TypeScript units plus integration, and 14 Python regressions.

## Remaining gates

Railway source/build, real S3/protocol checks, both two-minute soaks, persistence redeploy, rendered template/button and cleanup passed. The [published template](https://railway.com/deploy/varve) has no permanent demo; see [machine-readable evidence](evidence/railway-template.json) and the [wrapper validation record](https://github.com/monotykamary/railway-template-varve/blob/main/VALIDATION.md). The npm package is published and its clean registry install, strict declarations, browser bundle, SHA/SRI and 19-row/bigint/idempotency/recovery probe passed. Both Rust crate tarballs verify, but crates.io returned HTTP 400 requiring a verified account email; neither Rust crate is published and no registry-installed Rust probe is claimed. See [publication receipts and unblock steps](REGISTRY_RELEASE.md). Experimental single-node and DuckDB-alpha limitations remain unchanged.
