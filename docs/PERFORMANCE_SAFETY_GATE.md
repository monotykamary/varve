...
| Native SQL reuse | Bounded reusable native runtime, exact per-query authority/data, no stale callbacks/snapshots, cancellation/teardown safety, budget admission, output parity | Default-off isolated source candidate; callback-detachment/runtime qualification underway |
...
| Release checkpoint | Source-bound tests, direct probes, strict lint/format, independent review, exact committed artifact scope, commit and push verification | Safety/client/benchmark checkpoint qualified; commit review/push in progress |
